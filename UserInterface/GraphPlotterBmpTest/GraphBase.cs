...
namespace UserInterface.GraphPlotter
{
  ...
  public partial class GraphBase : AnalogBase
  {
    ...
    [Category("Graph Options"), Description("Buffer Size for graph min 500 max 50000 ")]
    public int BufferSize
    {
      get { return _bufferSize; }
      set
      {
        _bufferSize = Functions.MaxMinControl(value, 500000, 500);
      }
    }
    ...
    internal void TrimPoints(ref List<PointF> PointList, float Xmin, float Xmax, int bufferSize)
    {
      if (PointList.Count > bufferSize)
        PointList.RemoveRange(0, PointList.Count - bufferSize);
      if (PointList != null && PointList.Count > 3)
      {
        PointList = PointList.Where(pt => pt.X <= Xmax && pt.X >= Xmin).ToList();
        //PointList = PointList.Where(pt =>  pt.X >= Xmin).ToList();
      }
    }
    ...
    private void DrawHeader(Graphics graphics, string header, Font headerFont, Rectangle graphArea, Color stringColor)
    {
