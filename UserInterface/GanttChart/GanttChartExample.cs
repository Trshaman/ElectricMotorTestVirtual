...
{
  public class GanttChartExample : UserControl
  {
    ...
    public GanttChartExample()
    {
      ...
      this.MouseWheel += GanttChart_MouseWheel;
    }
    ...
    public void AddChartBar(BarInformation barInfo, object barValue)
    {
      ChartBarDate bar = new ChartBarDate();
      bar.Text = barInfo.RowText;
      bar.Value = barValue;
      bar.StartValue = barInfo.FromTime;
      bar.EndValue = barInfo.ToTime;
      bar.Color = barInfo.Color;
      bar.HoverColor = barInfo.HoverColor;
      bar.RowIndex = barInfo.Index;
      bars.Add(bar);

      SetBarStartLeft(barInfo.RowText);
    }
    ...
    private void DrawBars(Graphics grfx, bool ignoreScrollAndMousePosition = false)
    {
      ...
      //if ( shownHeaderList[1].Time < shownHeaderList[0].Time) 
      //  timeBetween = shownHeaderList[1].Time - shownHeaderList[0].Time;
      //else
      //  timeBetween = shownHeaderList[0].Time - shownHeaderList[1].Time;
